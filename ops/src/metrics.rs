use {
    das_bubblegum::metrics::{
        BUBBLEGUM_PROGRAM_TRANSFORMER_ERROR_COUNT, BUBBLEGUM_RPC_GET_SIGNATURES_FOR_ADDRESS_COUNT,
        BUBBLEGUM_RPC_GET_SIGNATURES_FOR_ADDRESS_TOTAL_COUNT, BUBBLEGUM_RPC_GET_TRANSACTION_COUNT,
        BUBBLEGUM_TREE_GAP_COUNT,
    },
...
pub fn run_metrics_server(address: String) -> anyhow::Result<()> {
    ...
    REGISTER.call_once(|| {
        ...
        register!(BUBBLEGUM_RPC_GET_TRANSACTION_COUNT);
        register!(BUBBLEGUM_RPC_GET_SIGNATURES_FOR_ADDRESS_COUNT);
        register!(BUBBLEGUM_RPC_GET_SIGNATURES_FOR_ADDRESS_TOTAL_COUNT);
    });
