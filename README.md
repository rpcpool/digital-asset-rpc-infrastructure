...
super-fast querying and searching, and serves the merkle proofs needed to operate over compressed nfts.
...
2. Api -> A JSON Rpc api that serves Metaplex objects. This api allows filtering, pagination, and searching over Metaplex data. This data includes serving the merkle proofs for the compressed NFTs system. It is intended to be run right alongside the Solana RPC and works in much the same way. Just like the solana RPC takes data from the validator and serves it in a new format, this api takes data from the validator and serves it.
...
- A Redis Cluster (Stream Optimized) - The example messaging system is a lightweight redis deployment that supports the streaming configuration.
...
This repository houses Helm Charts, Docker files, and Terraform files to assist in deploying the example infrastructure.

### Developing

#### Prerequisites:

You must clone the https://github.com/metaplex-foundation/blockbuster repo. This is unpublished for now due to active development in approximately 1,000 branches and the need for serious mathematics to avoid dependency hell.

Because this is a multi-component system, the easiest way to develop or locally test this system is with Docker, but developing locally without Docker is also possible.
...
> [!Note]
> The current SeaORM types were generated using version 0.9.3. Unless you want to upgrade, you can install using `cargo install sea-orm-cli --version 0.9.3`.
...
INGESTER_RPC_CONFIG: '{url="http://validator:8899", commitment="finalized"}' # your solana validator or same network rpc, if local, you must use your solana instance running locally
```
...
When making changes, you will need to stop the cargo process and rerun it. Someday, we will have auto-rebuild for local cargo items, but for now, you are on your own.
...
For production, you should separate the components.

### Developing With Docker

Developing with Docker is much easier, but it has some nuances to it. This test docker compose system relies on a programs folder being accessible. This folder needs to have the shared object files for the following programs
...
This script grabs all the code for these programs and compiles it, and chucks it into your programs folder. Grab some coffee because this will take a while/
If you encounter permission errors, use `sudo` to delete the programs directory and start again.
...
We use `docker-compose` to build the multi-container Docker application. On some systems, it's `docker compose`.
...
When making changes, you will need to run `docker compose up --build --force-recreate` again to get the latest changes.
...
- This script is not all-encompassing. It is intended to automate some basic routine tests that were previously performed manually. The reason this test is not added to CI is that it requires a more powerful system to run the Docker application, which contains the no-vote Solana validator.
- The test sequences are in `.scenario` files, but instead of sending those files to the `txn_forwarder` directly (which supports the file format), we parse them out and send them individually using the `single` parameter. This is because using the `.scenario` file directly results in a random ordering of the transactions, and we are explicitly trying to test them going forward and in reverse.
- In general, the expected database results are the same when running the transactions forward and backwards. However, for assets that are decompressed, this is not true because we don't index some of the asset information from Bubblegum mint indexing if we already know the asset has been decompressed. We instead let Token Metadata account based indexing fill in that information. This test script does not account for this, so the results differ when running these sequences in reverse. The differing results are reflected in test files with the `_reverse` suffix.
...
Once everything is working, you can see that there is an api being served on
...
Here are the metrics that various parts of this system expose;
